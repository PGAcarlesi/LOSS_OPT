...
class GVI_GP_alpha(gpflow.models.BayesianModel):
    def __init__(self, X, y, Kernel, objective="GVI", loss_type="MSE",
                 Sigma_f_structure="c_f", lambda_method="constant",
                 c_f_calibrating=False, inducing=None, lambda_pred_lev=0.95,
        ...
        # alpha parameter
        trainable_alpha = False if objective == "GVI" else True
        self.alpha = gpflow.Parameter(alpha, trainable=trainable_alpha)

        # sigma parameter (for non-MSE)
        if loss_type != "MSE":
            self.sigma = gpflow.Parameter(sigma, transform=gpflow.utilities.positive())

        # Sigma_f scaling
        if Sigma_f_structure == "c_f":
            trainable_c_f = not c_f_calibrating
            self.c_f = gpflow.Parameter(1.0, transform=gpflow.utilities.positive(), trainable=trainable_c_f)

        # lambda
        if lambda_method == "constant":
            self.lambda_i = tf.constant(1.0, dtype=tf.float64)
        elif lambda_method == "predictive" and objective == "predictive":
            self.lambda_i = gpflow.Parameter(1.0, transform=gpflow.utilities.positive(), trainable=True)
        else:
            raise ValueError("Unknown lambda_method")
        self.lambda_method = lambda_method

        self.test_size = 0.1  # only used for predictive objective

    def stable_solve(self, mat, rhs):
        jitter = 1e-6 * tf.eye(tf.shape(mat)[0], dtype=mat.dtype)
    ...
    def compute_sigma_f(self, X):
        if self.Sigma_f_structure == "c_f":
            ...
            # adaptive jitter
            jitter = 1e-6 * tf.reduce_max(tf.linalg.diag_part(K))

            if self.loss_type == "MSE":
                # Solve (K + I/lambda_i) x = K instead of inverting
                mat = K + eye / self.lambda_i + jitter * eye
                Sigma_f = self.lambda_i * tf.linalg.cholesky_solve(tf.linalg.cholesky(mat), K)
            else:
                mat = K + (self.sigma ** 2 / self.lambda_i) * eye + jitter * eye
                Sigma_f = (self.sigma ** 2 / self.lambda_i) * tf.linalg.cholesky_solve(tf.linalg.cholesky(mat), K)
        else:
            # sparse / inducing version
            K = self.kernel(X, X)
            ...
            jitter = 1e-6 * tf.reduce_max(tf.linalg.diag_part(K_uu))
            K_uu_chol = tf.linalg.cholesky(K_uu + jitter * tf.eye(K_uu.shape[0], dtype=tf.float64))
            K_uu_inv_K_uf = tf.linalg.cholesky_solve(K_uu_chol, K_uf)
            Sigma_f = K_fu @ K_uu_inv_K_uf
    ...
    def GVI_step(self):
        ...
        Sigma_a = self.alpha * K + (1.0 - self.alpha) * Sigma_f + jitter * eye
        Sigma_a_inv = self.stable_solve(Sigma_a, eye)

        if self.loss_type == "MSE":
            mat = self.alpha * Sigma_a_inv + self.lambda_i * eye
            q1 = self.stable_solve(mat, eye)
            q2 = self.lambda_i * self.y
            mu_f = tf.matmul(q1, q2)
            term1 = 0.5 * tf.transpose(self.y - mu_f) @ (self.y - mu_f) * self.lambda_i
            term1 += 0.5 * tf.linalg.trace(Sigma_f) * self.lambda_i
        else:
            mat = self.alpha * Sigma_a_inv + (self.lambda_i / self.sigma ** 2) * eye
            q1 = self.stable_solve(mat, eye)
            q2 = (self.lambda_i / self.sigma ** 2) * self.y
            mu_f = tf.matmul(q1, q2)
            term1 = 0.5 * tf.transpose(self.y - mu_f) @ (self.y - mu_f) * (self.lambda_i / self.sigma ** 2)
            term1 += 0.5 * tf.linalg.trace(Sigma_f) * (self.lambda_i / self.sigma ** 2)

        # Log-determinants for GVI
        ld_Sigma_a = 2.0 * tf.reduce_sum(tf.math.log(tf.linalg.diag_part(tf.linalg.cholesky(Sigma_a))))
        ld_Sigma_f = 2.0 * tf.reduce_sum(tf.math.log(tf.linalg.diag_part(tf.linalg.cholesky(Sigma_f + jitter * eye))))
        ld_K = 2.0 * tf.reduce_sum(tf.math.log(tf.linalg.diag_part(tf.linalg.cholesky(K + jitter * eye))))

        term2 = 0.5 * self.alpha * tf.transpose(mu_f) @ Sigma_a_inv @ mu_f
        term3 = -1.0 / (2.0 * (self.alpha - 1.0 + 1e-6)) * (
                    ld_Sigma_a + (self.alpha - 1.0) * ld_Sigma_f - self.alpha * ld_K)
    ...
    def Pred_step(self):
        ...
        sigma_p = tf.linalg.diag_part(tf.linalg.inv(
            K_star_star - K_star @ K_inv @ tf.transpose(K_star) + K_star @ K_inv @ Sigma_f @ tf.transpose(
                K_star @ K_inv)))
        ...
        if self.loss_type == "MSE":
            ...
            eye = tf.eye(tf.shape(K_star_star)[0], dtype=K_star.dtype)
            jitter = eye * 1e-6
            sigma_p = tf.linalg.inv(sigma_p+jitter)
...
class GVI_SGPR_alpha(gpflow.models.BayesianModel):
    def __init__(self, X, y, inducing, N_u, loss_type="MSE", structure="Diag", alpha=0.5, test_size=0.1):
        super().__init__()
        self.X = X  # [N, D]
        self.y = y  # [N, 1]
        self.inducing_variable = inducing
